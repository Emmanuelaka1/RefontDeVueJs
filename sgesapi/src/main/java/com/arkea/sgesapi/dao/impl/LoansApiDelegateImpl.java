...
/**
 * Implémentation du delegate Loans API.
 * <p>
 * Orchestre le flux complet :
 *   1. Appel SIGAC via LoansApi (mock en dev, REST en val/rec/hml/prod)
...
public class LoansApiDelegateImpl implements LoansApiDelegate {
    ...
    public LoansApiDelegateImpl(LoansApi loansApi, PersonnesService personnesService) {
        this.loansApi = loansApi;
        this.personnesService = personnesService;
    }

    /**
     * Récupère un prêt depuis SIGAC, le convertit en DossierConsultationDto
     * et enrichit les noms des participants via opentopazservice.
     *
     * @param id numéro contrat souscrit (Topaze Contrat ID)
     * @return ResponseEntity contenant le DossierConsultationDto, ou 404/500
     */
    @Override
    public ResponseEntity<DossierConsultationDto> searchLoans(String id) {
        log.info("Loans API — getLoan contratId={}", id);

        try {
            ResponseEntity<CommonLoan> resLoan = loansApi.getLoan(id);

            if (resLoan.getStatusCode() != HttpStatus.OK || resLoan.getBody() == null) {
                log.warn("Prêt non trouvé : {}", id);
                return ResponseEntity.notFound().build();
            }

            // CommonLoan → DossierConsultationDto (modèle interne)
            DossierConsultationDto dossier = toDossierConsultationDto(resLoan.getBody());
